...
import { Page, URLCache } from './model'

const scanDir = async (directory: string, parent: string = ''): Promise<URLCache> => {
  ...
  const children = await Promise.all(entries.map(async entry => {
    ...
    if (entry.isFile() && path.extname(entry.name) === '.md') {
      ...
      const defaultTitle = isSection ? path.basename(parent) : path.basename(localPath, '.md')
...
const scanContentDir = async (contentDir: string): Promise<URLCache> => {
